                                ...
                                Vector<double>       &u_out)
{
          ...
          RHSAssemblyCopyData                                  &copy_data)
  {
    ...
    for (unsigned int q = 0; q < n_q_points; ++q)
      scratch.rhs_values[q] =
          rhs_function.value(scratch.fe_values.quadrature_point(q));
  ...
  // Only reinitialize preconditioner when the matrix changes (i.e., when dt
  // changes)
                                  ...
                                  const unsigned int marked_coarsen) const
{
  ...
  log << timestep_number << "," << time << "," << triangulation.n_active_cells()
...
HeatEquation<dim>::run()
{
  ...
  if (write_vtk && !times_and_names.empty())
  {
    ...
    std::cout << "[" << run_name << "] PVD master file written to "
              << output_dir << "/solution.pvd\n";
  }
}
