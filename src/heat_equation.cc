                                ...
                                TrilinosWrappers::MPI::Vector&       u_out)
{
  ...
  // condense() fails on distributed vectors when parents are ghosts.
  {
    ...
    RightHandSide<dim> rhs_func(
        rhs_N, rhs_sigma, rhs_a, rhs_x0, source_intensity);
    ...
    for (const auto& cell : dof_handler.active_cell_iterators())
    {
      if (cell->is_locally_owned())
      {
        ...
        // parents) AND parallel assembly (compressing contributions to ghosts)
        // correctly.
        constraints.distribute_local_to_global(
            cell_rhs, local_dof_indices, forcing);
      }
    }
                                  ...
                                  const unsigned int marked_coarsen) const
{
  if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0)
  {
    ...
    log << timestep_number << "," << time << ","
        ...
        << "," << triangulation.n_levels() << "," << marked_refine << ","
        << marked_coarsen << "\n";
  }
}
...
template <int dim>
void
HeatEquation<dim>::log_time_event(const double t_now,
                                  const double dt_now,
                                  const int    accepted,
...
HeatEquation<dim>::solve_timestep_heuristic()
{
  ...
  VectorTools::create_right_hand_side(
      dof_handler, QGauss<dim>(fe.degree + 1), rhs_function, rhs_new);

  TrilinosWrappers::MPI::Vector rhs_old(locally_owned_dofs, mpi_communicator);
  rhs_function.set_time(time);
  VectorTools::create_right_hand_side(
      dof_handler, QGauss<dim>(fe.degree + 1), rhs_function, rhs_old);
