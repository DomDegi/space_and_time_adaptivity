...
main(int argc, char* argv[])
{
  try
  {
    ...
    if (sigma <= 0.0)
    {
      std::cerr << "Error: Source width (sigma) must be strictly positive.\n";
      return 1;
    }
    ...
    if (user_cp <= 0.0)
    {
      std::cerr << "Error: Specific Heat (c_p) must be strictly positive.\n";
      return 1;
    }
    if (user_k <= 0.0)
    {
    ...
    if (user_tol <= 0.0)
    {
      std::cerr << "Error: Time step tolerance must be strictly positive.\n";
      return 1;
    }
    if (user_dt_min <= 0.0)
    {
