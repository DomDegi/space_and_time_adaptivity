...
public:
  ...
  struct RunStats
  {
    ...
    reset();
    void
    sample_dofs_and_cells(const unsigned int ndofs, const unsigned int ncells);
    void
    register_cg_iterations(const unsigned int it);
    void
    register_time_step_attempt(const double dt, const bool accepted);
    double
    dt_mean() const;
    double
    dof_mean() const;
...
private:
  ...
  void
  log_mesh_event(const unsigned int marked_refine,
                 const unsigned int marked_coarsen) const;

  void
  log_time_event(const double t_now,
                 const double dt_now,
                 const int    accepted,
  ...
  bool output_at_each_timestep;  ///< Output at each timestep (debugging) or at
                                 ///< fixed intervals
  double output_time_interval;   ///< Time interval for output when not
                                 ///< outputting each timestep
